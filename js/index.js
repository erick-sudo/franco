...
let cakeModel;
const cakeWithFlame = new THREE.Group();
const allFlameMaterials = [];
const flames = [];
const cakeRotSpeed = 0.003;

const spheres = [];
// const ballColors = [
//   0x00bfff, 0xff00ff, 0x7cfc00, 0xdc143c, 0x7fff00, 0x00bfff, 0x00ffff,
//   0xff0000,
// ];
const ballColors = [
  0xffa500, // Orange
  ...
  0xffd700, // Golden Orange
];
const balloonCount = 100;
const balloonSpeed = 0.0001;

const text = "Happy Birthday! Dear Franciscah! --- ^o^ --- ";
// const textColors = [
//   0x00bfff, 0xff00ff, 0x7cfc00, 0xdc143c, 0x7fff00, 0x00bfff, 0x00ffff,
//   0xff0000,
// ];
...
const textScaleSize = 0.3; //delta scale for letters
const textAnimeColors = []; // Index of current color for each letter
const textRotSpeed = 0.2; // Text world rotation speed. in degree
const textInitialSize = 120; // Initial font size for text
const textYScope = 5; // Text Sin wave height.
const textYSpeed = 10; // Text Sin wave speed
const textBevelThickness = 3; // Text bevel thickness
const textBevelSize = 3; // Text bevel size
const textFont = "fonts/gentilis_regular.typeface.json"; // textFont  ::Only ThreeJS fonts are available
const distanceToText = 1000; // Distance from cakemodel to text
const textSizeInAngle = 8; // Angle width for each letter.
let textRot = 0; // current text rotation.
const textColorTransformSpeed = 50; // text color transform speed.  !CAUTION: The smaller the faster
const textObjects = [];
...
function addBalloons() {
  ...
  var lineMat = new THREE.LineBasicMaterial({
    // balloon tail material: you can customize *color*
    color: 0xaaaaaa,
    linewidth: 5,
    transparent: true,
    opacity: 0.6,
  ...
  for (var i = 0; i < balloonCount; i++) {
    ...
    mesh.scale.x = mesh.scale.y = mesh.scale.z = Math.random() * 0.1 + 0.3;
...
function placeName() {
  new THREE.FontLoader().load(
    "fonts/playwrite_vn_thin_regular.typeface.json",
    function (font) {
      // Name text geometry
      const nameGeometry = new THREE.TextGeometry("Francie", {
        font: font,
        size: 1.8,
        depth: 0.08,
        height: 0.08,
      ...
      nameMesh.position.set(-3, -8, 9);
      nameMesh.rotation.x = -Math.PI / 2;
      nameMesh.rotation.z = 0.34;
      cakeWithFlame.add(nameMesh);
    }
  );
}
...
function placeFlames() {
  ...
  for (var i = 0; i < 16; i++) {
    a =
      THREE.Math.degToRad((360 / 16) * i + initialAngle) + cakeModel.rotation.z;
    x = r * Math.cos(a);
    z = r * Math.sin(a);
    flame(true, x, z);
  }
...
function insertText() {
  ...
  loader.load(textFont, function (font) {
    text.split("").map((letter, i) => {
      ...
      textMesh.position.x =
        distanceToText * Math.sin(rot) + cakeModel.position.x;
      textMesh.position.z =
        distanceToText * Math.cos(rot) * cakeModel.position.z;
...
var render = function () {
  ...
  text.split("").map((letter, i) => {
    ...
    textObjects[i].position.x =
      distanceToText * Math.sin(rot) + cakeModel.position.x;
    textObjects[i].position.z =
      distanceToText * Math.cos(rot) * cakeModel.position.z;
    textObjects[i].position.y =
      cakeModel.position.y + Math.sin(rot * textYSpeed) * textYScope;
  ...
  dx = (mouseX - camera.position.x) * 0.05;
  dy = (-mouseY - camera.position.y) * 0.05;
